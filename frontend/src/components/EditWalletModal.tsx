...
export default function EditWalletModal({ open, wallet, groups, onClose, onSaved }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await walletsApi.updateWallet(wallet.id, {
        name: walletName,
        walletType,
        groupId,
      });
      ...
      toast.error(err instanceof Error ? err.message : t("modal.updateFailed"));
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-4">
      ...
      <div className="relative w-full max-w-lg rounded-t-3xl md:rounded-3xl border border-white/15 bg-ink-900/95 backdrop-blur-xl shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
          <h2 className="font-display font-bold text-lg">{t("modal.editTitle")}</h2>
        ...
        <form onSubmit={(e) => void handleSubmit(e)} className="p-5 space-y-4">
          ...
          <button type="submit" disabled={saving} className="btn-primary w-full !py-3 disabled:opacity-50">
            {saving ? t("saving") : t("modal.saveChanges")}
          </button>
        </form>
      </div>
    </div>
