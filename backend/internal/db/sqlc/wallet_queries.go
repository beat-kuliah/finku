...
const walletCols = `id, user_id, name, wallet_type, icon, balance, group_id, archived_at, created_at, updated_at`

func scanWallet(row interface{ Scan(dest ...any) error }) (Wallet, error) {
	var w Wallet
	err := row.Scan(
		&w.ID, &w.UserID, &w.Name, &w.WalletType, &w.Icon, &w.Balance, &w.GroupID,
...
func (q *Queries) InsertWallet(ctx context.Context, userID uuid.UUID, name, walletType string, icon *string, groupID *uuid.UUID) (Wallet, error) {
	row := q.pool.QueryRow(ctx, `
INSERT INTO wallets (user_id, name, wallet_type, icon, group_id)
VALUES ($1, $2, $3, $4, $5)
RETURNING `+walletCols, userID, name, walletType, icon, groupID)
...
func (q *Queries) UpdateWallet(ctx context.Context, userID, walletID uuid.UUID, name, walletType string, icon *string, groupID *uuid.UUID) (Wallet, error) {
	row := q.pool.QueryRow(ctx, `
UPDATE wallets SET name = $3, wallet_type = $4, icon = $5, group_id = $6, updated_at = NOW()
WHERE id = $2 AND user_id = $1
RETURNING `+walletCols, userID, walletID, name, walletType, icon, groupID)
	return scanWallet(row)
}
