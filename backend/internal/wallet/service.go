...
func (s *Service) Create(ctx context.Context, userID uuid.UUID, name, walletType string, icon *string) (map[string]any, error) {
	...
	w, err := s.q.InsertWallet(ctx, userID, name, walletType, icon)
	if err != nil {
		return nil, err
	}
...
func (s *Service) Update(ctx context.Context, userID, walletID uuid.UUID, name, walletType string, icon *string) (map[string]any, error) {
	name = strings.TrimSpace(name)
	if name == "" {
		return nil, httpx.SvcErr(http.StatusBadRequest, "VALIDATION", "Nama dompet wajib diisi.")
	}
	...
	w, err := s.q.UpdateWallet(ctx, userID, walletID, name, walletType, icon)
	if err != nil {
