...
func Load() (*Config, error) {
	...
	c := &Config{
		...
		GlobalRateLimitMax: mustParseInt(getEnv("GLOBAL_RATE_LIMIT_REQUESTS", "100"), 100),
		GlobalRateLimitWin: mustParseDur(getEnv("GLOBAL_RATE_LIMIT_WINDOW", "1s"), time.Second),
		LockoutMax:         mustParseInt(getEnv("LOCKOUT_MAX_ATTEMPTS", "5"), 5),
		LockoutWindow:      mustParseDur(getEnv("LOCKOUT_WINDOW", "1m"), time.Minute),
